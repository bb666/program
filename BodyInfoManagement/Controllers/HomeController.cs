...
namespace BodyInfoManagement.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<ActionResult> Pass(ChangePasswordModel passwordBag)
        {
            if (passwordBag.NewPassword != passwordBag.ConfirmPassword)
            {
                ModelState.AddModelError("", "新密码和确认密码不匹配");
                return View(passwordBag);
            }

            if (Session["NowUserId"] != null)
            {
                string id = Session["NowUserId"].ToString();
                var findStudent = await db.Students.FindAsync(id);
                if (findStudent != null &&
                    findStudent.StudentPassword == EncipherSha(findStudent.SaltCode + passwordBag.OldPassword))
                {
                    findStudent.StudentPassword = EncipherSha(findStudent.SaltCode + passwordBag.NewPassword);
                    await db.SaveChangesAsync();
                    return RedirectToAction("Index");
                }

                var findTeacher = await db.Teachers.FindAsync(id);
                if (findTeacher != null &&
    findTeacher.TeacherPassword == EncipherSha(findTeacher.SaltCode + passwordBag.OldPassword))
                {
                    findTeacher.TeacherPassword = EncipherSha(findStudent.SaltCode + passwordBag.NewPassword);
                    await db.SaveChangesAsync();
                    return RedirectToAction("Index");
                }
            }
                ...
                case 1:
                    {
                        ...
                        {
                            WriteSession(1, find.StudentId, find.StudentAuth);
                ...
                case 2:
                    {
                        ...
                        if (find != null)
                        {
                            ...
                            {
                                WriteSession(2, find.AdministratorId, true);
